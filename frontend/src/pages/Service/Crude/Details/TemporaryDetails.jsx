// TemporaryDetails.jsx - Version française
...
export default function TemporaryDetails() {
      ...
      try {
        ...
        try {
          const contractCheck = await axios.get(
            `http://localhost:5454/api/badges/temporary/${badgeNum}/contract/exists`,
            { withCredentials: true }
          );
  ...
  const determineUnifiedStatus = (badgeData) => {
    ...
    // For TEMPORARY badges, check if completed first
  ...
  const downloadContract = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5454/api/badges/temporary/${badgeNum}/contract`,
        {
          withCredentials: true,
          responseType: 'blob'
        }
            ...
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
              ...
              <div className="flex items-center">
                ...
                <p className="ml-3 text-sm text-gray-600">
                  {statusText === 'Actif' && 'Ce badge temporaire est actuellement actif et valide.'}
                  {statusText === 'Expiré' && 'Ce badge temporaire a expiré et n\'est plus valide.'}
                  {statusText === 'En traitement' && 'Ce badge temporaire est actuellement en cours de traitement.'}
                </p>
              </div>
            </div>
          </div>
