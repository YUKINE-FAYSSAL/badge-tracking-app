// PermanentList.jsx - Version Française Unifiée - Updated Status System
...
export default function PermanentList() {
  ...
  const fetchBadges = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:5454/api/badges/permanent', {
  ...
  const handleDelete = async (badgeNum) => {
    ...
    try {
      await axios.delete(`http://localhost:5454/api/badges/permanent/${badgeNum}`, {
        withCredentials: true
      });
  ...
  const getValidityStatus = (badge) => {
    ...
    if (badge.validity_duration) {
      const returnDate = new Date(badge.gr_return_date);
      if (badge.validity_duration === '1 year') {
        validityEnd = new Date(returnDate.setFullYear(returnDate.getFullYear() + 1));
      } else if (badge.validity_duration === '3 years') {
        validityEnd = new Date(returnDate.setFullYear(returnDate.getFullYear() + 3));
      } else if (badge.validity_duration === '5 years') {
        validityEnd = new Date(returnDate.setFullYear(returnDate.getFullYear() + 5));
      } else {
        validityEnd = new Date(returnDate.setFullYear(returnDate.getFullYear() + 1));
      }
    } else if (badge.validity_end) {
      validityEnd = new Date(badge.validity_end);
    } else {
      return { 
        status: 'unknown', 
        text: 'Validité inconnue', 
        color: 'text-gray-600', 
        bg: 'bg-gray-100' 
      };
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-800">Badges Permanents</h1>
        <Link
                  ...
                  <tr key={badge.badge_num} className={getRowClass(badge)}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        <Link
                          to={`/service/permanent/view/${badge.badge_num}`}
                        ...
                        <Link
                          to={`/service/permanent/edit/${badge.badge_num}`}
